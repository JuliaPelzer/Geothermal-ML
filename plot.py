...
def plot_comparison_small(input, pred, target):
    fig, axes = plt.subplots(1, 3, figsize=(12, 4))
    imsize = 17
    #new_pred = pred[:,:,24:41, 24:41]
    #new_target = target[:,24:41, 24:41]
    # vmin = 10.0
    # vmax = 14.0
    ...
    vmin = target.min()
    vmax = target.max()
    ax_pred = plot_velocity_temperature_ax(axes[0], input, pred.squeeze(), vmin, vmax, imsize, cmap="Reds")
